...
if ( ! function_exists( 'ampnews_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function ampnews_posted_on() {
		...
		$posted_on = sprintf(
			wp_kses(
				...
				_x( '<span class="screen-reader-text">Posted on</span> %s', 'post date', 'ampnews' ),
				array(
					'span' => array(
						'class' => array(),
					),
	...
	function ampnews_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'ampnews' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'ampnews' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'ampnews' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'ampnews' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'ampnews' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'ampnews_post_thumbnail' ) ) :
	/**
	 * Displays an optional post thumbnail.
	 */
	function ampnews_post_thumbnail() {
		if ( post_password_required() || is_attachment() || ! has_post_thumbnail() ) {
			return;
		}

		// TODO: Create custom markup for ampnews's specific image sizes.
	}
endif;

if ( ! function_exists( 'ampnews_branding_tag' ) ) :
	/**
	 * Outputs the branding in header as h1 or p, depending on the current view.
	 */
	function ampnews_branding_tag() {
 ...
 */
function ampnews_the_post_attributes( $attributes = array() ) {
